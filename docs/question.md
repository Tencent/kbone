...
* 开发资源不足，比如小程序内置组件的支持目前只支持了 `picker` 组件等，用法可阅读[开发文档](./quickstart.md#使用小程序内置组件)，后续会慢慢补齐。
* 小程序本身的运行机制限制。
...
Q：能否使用小程序原生组件？
A：目前代码里预埋进了能力，但是仅支持了 `picker` 组件等，用法可阅读[开发文档](./quickstart.md#使用小程序内置组件)，后续会慢慢补齐。
