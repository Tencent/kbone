...
module.exports = {
    properties: [{
        ...
        name: 'delta',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('delta'), 10)
        ...
        name: 'hoverStartTime',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('hover-start-time'), 10)
            return !isNaN(value) ? value : 50
        },
    }, {
        name: 'hoverStayTime',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('hover-stay-time'), 10)
