...
module.exports = {
    properties: [{
        ...
        name: 'damping',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('damping'), 10)
            return !isNaN(value) ? value : 20
        },
    }, {
        name: 'friction',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('friction'), 10)
        ...
        name: 'scaleMin',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('scale-min'), 10)
            return !isNaN(value) ? value : 0.5
        },
    }, {
        name: 'scaleMax',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('scale-max'), 10)
            return !isNaN(value) ? value : 10
        },
    }, {
        name: 'scaleValue',
        get(domNode) {
            const value = parseFloat(domNode.getAttribute('scale-value'), 10)
