...
function dealWithPage(evt, window, value) {
    ...
    if (value === 'webview') {
        ...
        var options = { url: `/pages/webview/index?url=${encodeURIComponent(url)}` }
        if (type === 'jump') wx.redirectTo(options)
        else if (type === 'open') wx.navigateTo(options)
    } else if (value === 'error') {
        console.error(`page not found: ${evt.url}`)
    } else if (value !== 'none') {
        var targeturl = `${window.location.origin}/redirect?url=${encodeURIComponent(url)}`
        var options = { url: `/pages/${value}/index?type=${type}&targeturl=${encodeURIComponent(targeturl)}` }
        if (type === 'jump') wx.redirectTo(options)
        else if (type === 'open') wx.navigateTo(options)
    }
}
...
Page({
    ...
    onLoad(query) {
        ...
        // 处理 selectorQuery 获取
        this.window._$createSelectorQuery = () => {
            return wx.createSelectorQuery().in(this)
        }

        // 处理 intersectionObserver 获取
        this.window._$createIntersectionObserver = options => {
            return wx.createIntersectionObserver(this, options)
        }
    ...
    onShareAppMessage(data) {
        if (this.app.onShareAppMessage) {
            ...
            return this.app.onShareAppMessage(data, currentPagePath)
        }
    },
