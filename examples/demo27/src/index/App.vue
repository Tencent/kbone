...
export default {
  ...
  methods: {
    ...
    onWxpicker3ColumnChange(evt) {
      const value = this.wxPicker.value3
      const range = this.wxPicker.range3
      this.$set(value, evt.detail.column, +evt.detail.value)
      switch (evt.detail.column) {
        case 0:
          switch (value[0]) {
            case 0:
              this.$set(range, 1, ['扁性动物', '线形动物', '环节动物', '软体动物', '节肢动物'])
              this.$set(range, 2, ['猪肉绦虫', '吸血虫'])
              break
            case 1:
              this.$set(range, 1, ['鱼', '两栖动物', '爬行动物'])
              this.$set(range, 2, ['鲫鱼', '带鱼'])
              break
          }
          this.$set(value, 1, 0)
          this.$set(value, 2, 0)
            ...
            case 0:
              switch (value[1]) {
                  ...
                  case 1:
                    ...
                    break
                  case 2:
                    this.$set(range, 2, ['蚂蚁', '蚂蟥'])
                    break
                  case 3:
                    this.$set(range, 2, ['河蚌', '蜗牛', '蛞蝓'])
                    break
                  case 4:
                    this.$set(range, 2, ['昆虫', '甲壳动物', '蛛形动物', '多足动物'])
                    break
                }
                break;
              case 1:
                switch (value[1]) {
                  case 0:
                    ...
                    break
                  case 1:
                    this.$set(range, 2, ['青蛙', '娃娃鱼'])
                    break
                  case 2:
                    this.$set(range, 2, ['蜥蜴', '龟', '壁虎'])
                    break
                }
                break
            }
    ...
    onWxpicker4ColumnChange(evt) {
      const range = this.wxPicker.range4
      const value = this.wxPicker.value4
      this.$set(value, evt.detail.column, +evt.detail.value)
      if (evt.detail.column === 0) {
        if (value[0] === 0) {
          this.$set(range, 1, [{name: '中国'}, {name: '日本'}])
        } else if (value[0] === 1) {
          this.$set(range, 1, [{name: '北京'}, {name: '东京'}])
        }
        this.$set(value, 1, 0)
      }
    },
